...
print("\n>>> BOOTING NATIVE HARDWARE PROFILER...", flush=True)
...
def run_debugger():
    try:
        print_header("VIBEVOICE ZERO-OVERHEAD PROFILER")
        ...
        if not os.path.exists(voice_path):
            print(f"\n[CRITICAL ERROR] Cannot find {voice_path}!")
            return
        ...
        prompt += f" Text input:\n Speaker 0: This is a strict hardware trace. We are proving that the CPU is bypassed entirely.\n Speech output:\n<|vision_start|>"
        
        input_ids = engine.tokenizer.encode(prompt, add_bos=False, encode_special_tokens=True, embeddings=[voice_mme])
        
        print_header("STARTING CHROME TRACE PROFILER")
        ...
        with torch.inference_mode():
            ...
            # --- MICRO-SYNC FIXES: Preallocate ExLlama GPU state to bypass CPU host-to-device transfers ---
            gpu_block_table = torch.arange(8192 // 256, dtype=torch.int32, device=device).unsqueeze(0)
            gpu_cache_seqlens = torch.tensor([input_ids.shape[1]], dtype=torch.int32, device=device)
            gpu_eos_id = torch.tensor([engine.speech_end_id], dtype=torch.long, device=device)
            
            try:
                # Prefill
                inputs_embeds_pos = engine.model.modules[0].forward(input_ids, {"indexed_embeddings": [voice_mme]})
                
                # By passing block_table and cache_seqlens directly, we bypass ExLlama's CPU tensor creation
                params_pos = {
                    "attn_mode": "flash_attn", 
                    "cache": cache_pos, 
                    "block_table": gpu_block_table, 
                    "cache_seqlens": gpu_cache_seqlens
                }
                ...
                print(f"[INFO] Running AR Loop. Trace will be saved to vibevoice_trace.json")
                ...
                with torch.profiler.profile(
                    ...
                    for chunk_start in range(0, 1500, chunk_size):
                        ...
                        for t in range(chunk_start, min(chunk_start + chunk_size, 1500)):
                            ...
                            # Increment GPU tensor natively (No CPU sync!)
                            gpu_cache_seqlens.add_(1)
                            tokens_gen += 1
                            
                            pred_id = logits_pos[0, -1, :].argmax()
                            chunk_preds.append(pred_id)
                            
                            # Compare against GPU tensor (No CPU sync!)
                            eos_flag.logical_or_(pred_id == gpu_eos_id)
                ...
                # --- END TORCH PROFILER ---
                prof.export_chrome_trace("vibevoice_trace.json")
        ...
        print("-" * 50)
        print(f"CPU Python Queue Time:      {total_cpu_queue_time:.4f} sec")
        print(f"PCIe Sync & Idle Time:      {total_pcie_sync_time:.4f} sec")
        print(f"True GPU Compute Time:      {total_gpu_hardware_time:.4f} sec")
        print("\n[SUCCESS] Trace file saved to vibevoice_trace.json")
        print("Open your browser, navigate to 'chrome://tracing', and load the JSON file to see the exact CPU/GPU execution timeline.")

    except Exception as e:
        print("\n[CRITICAL FAILURE] The profiler crashed:")
        traceback.print_exc()
