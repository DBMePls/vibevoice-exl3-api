...
print("\n>>> BOOTING HARDWARE PROFILER...", flush=True)
...
# Enable logging so we can see the engine boot
...
def run_debugger():
    try:
        ...
        print("[INFO] Loading engine...", flush=True)
        ...
        if not os.path.exists(voice_path):
            ...
            print("Please run this script from the root of the vibevoice-exl3-api directory.")
            return

        print(f"[INFO] Using reference voice: {voice_path}", flush=True)
        ...
        with torch.inference_mode():
            ...
            try:
                ...
                print(f"[INFO] AR Loop running with chunk size: {chunk_size}")
                ...
                for chunk_start in range(0, 1500, chunk_size):
                    ...
                    # --- START PCIe SYNC TIMER ---
                    t_sync_start = time.perf_counter()
                    is_eos = eos_flag.item() # <--- THE PCIe SYNC
                    total_pcie_sync_time += (time.perf_counter() - t_sync_start)
                    # --- STOP PCIe SYNC TIMER ---
        ...
        if gpu_utilization > 85:
            print("[CONCLUSION] PERFECT. Your CPU is no longer a bottleneck. The 3090 is running at max speed.")
        else:
            print("[CONCLUSION] Sub-optimal. Consider increasing chunk_size to 50 to hide more PCIe latency.")
